...
void loop() {
  ...
  if(state == -1){
    ...
    matrix.println("VS4"); 
    if (prevy_ctr1 > 200 && y_ctr1 < 200) state = 0;
    if (prevy_ctr2 > 200 && y_ctr2 < 200) state = 0;
  }
    ...
    matrix.drawLine(droppoint *4, 4, droppoint*4+2, 4, matrix.Color333(0, 7, 0));
    matrix.drawLine(droppoint *4, 30-board[6][droppoint]*4, droppoint*4+2, 30-board[6][droppoint]*4, matrix.Color333(0, 7, 0));
    ...
    /*制限時間*/
    sec = (millis()-setMillis)/1000;
    matrix.setCursor(40, 12); 
    if(sec!=presec) matrix.fillRect(40,12, 40, 8, matrix.Color333(0, 0, 0));
    matrix.print(5-sec);
    presec = sec;
    if(sec>=sec_limit) state = 4;
    
    if (prevx_ctr1 < 800 && x_ctr1 >= 800) {
      if (droppoint > 0) {
        Serial.println("Left");
        matrix.drawLine(droppoint *4, 30-board[6][droppoint]*4, droppoint*4+2, 30-board[6][droppoint]*4, matrix.Color333(0, 0, 0));
        droppoint--;
        matrix.drawLine(droppoint *4, 30-board[6][droppoint]*4, droppoint*4+2, 30-board[6][droppoint]*4, matrix.Color333(0, 7, 0));
      ...
      if (point_answer[0] == 0) {
        setMillis = millis();
        matrix.fillRect(40,12, 40, 8, matrix.Color333(0, 0, 0));
        matrix.fillRect(droppoint * 4, 1, 3, 3, matrix.Color333(7, 0, 0));
        player = -1;
      }
      else {
    ...
    */
    ...
    delay(500);
    matrix.fillRect(point_answer[1] * 4, , 3, 3, matrix.Color333(7, 7, 7));
  }
  ...
  /*TIME OVER*/
    ...
    matrix.println("TIMEOVER");
    matrix.setCursor(12, 18);
    if(player ==1)matrix.print(1);
    else matrix.print(2);
    matrix.println("P LOSE");
    delay(5000);
    state = -1;
    matrix.fillScreen(matrix.Color333(0, 0, 0));
  }
